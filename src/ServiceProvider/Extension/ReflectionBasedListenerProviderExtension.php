...
/**
 * Populate the reflection-based listener provider from configured listeners.
 *
 * @internal
 */
final readonly class ReflectionBasedListenerProviderExtension
{
    /**
     * Register configured reflection listeners with the provider.
     *
     * @param ContainerInterface $container container used to resolve listener services
     * @param ReflectionBasedListenerProvider $reflectionBasedListenerProvider provider to extend
     *
    ...
    public function __invoke(
        ...
        foreach ($configuredListeners->reflectionListeners() as $listener) {
            $reflectionBasedListenerProvider->listen(
                $this->resolveCallable($container, $listener->listener),
                $listener->eventType,
            );
        }
    }
    ...
    /**
     * Resolve a configured reflection listener to a callable value.
     *
     * @param ContainerInterface $container container used for service resolution
     * @param mixed $listener listener value or service identifier
